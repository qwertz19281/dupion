...
pub trait Deduper {
    fn dedup(&mut self, state: &'static RwLock<State>, opts: &'static Opts, driver: &mut impl Driver) -> AnyhowResult<()> {
        ...
        for (_,e) in &hash_groups {
            if e.size < opts.scan_size_min || e.size > opts.scan_size_max || e.entries.len() < 2 {continue;}
        ...
        let groups = hash_groups.iter()
            .filter_map(|(_,e)| {
                if e.size < opts.scan_size_min || e.size > opts.scan_size_max || e.entries.len() < 2 {return None;} //TODO proper min size option
