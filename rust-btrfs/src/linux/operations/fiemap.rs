...
pub fn get_file_extent_map_for_path_noloop<PathRef: AsRef<Path>>(
    file_path: PathRef,
) -> Result<Vec<FileExtent>, String> {
    let file_descriptor = FileDescriptor::open(file_path, libc::O_RDONLY)
        .map_err(|error| format!("Error opening file: {}", error))?;

    get_file_extent_map_noloop(file_descriptor.get_value())
}

pub fn get_file_extent_map(file_descriptor: libc::c_int) -> Result<Vec<FileExtent>, String> {
    ...
    loop {
        ...
        if extent_count != 0 {
            ...
            extent_count = c_file_extent_map.info.mapped_extents + 16;
        }
    }
    ...
    Ok(
        c_file_extent_map.extents[0..c_file_extent_map.info.mapped_extents as usize]
            .iter()
            .map(|c_file_extent| FileExtent {
...
pub fn get_file_extent_map_noloop(file_descriptor: libc::c_int) -> Result<Vec<FileExtent>, String> {
    let initial_extent_query = 128;

    let mut c_file_extent_map = get_c_file_extent_map(file_descriptor, initial_extent_query)?;

    if c_file_extent_map.info.mapped_extents + 16 > initial_extent_query {
        c_file_extent_map = get_c_file_extent_map(file_descriptor, c_file_extent_map.info.mapped_extents + 16)?;
    }
...
fn get_c_file_extent_map(
    ...
    let mut c_stat: libc::stat = unsafe { mem::zeroed() };

    let stat_result = unsafe { libc::fstat(file_descriptor, &mut c_stat as *mut libc::stat) };

    if stat_result != 0 {
        return Err("Error getting file size".to_owned());
    }
    ...
    let c_fiemap_info_slice: &mut [IoctlFiemap] =
        unsafe { slice::from_raw_parts_mut(mem::transmute(c_fiemap_info_buffer.as_mut_ptr()), 1) };

    let c_fiemap_info = &mut c_fiemap_info_slice[0];

    let c_fiemap_extents: &mut [IoctlFiemapExtent] = unsafe {
        slice::from_raw_parts_mut(
            mem::transmute(c_fiemap_extents_buffer.as_mut_ptr()),
    ...
    unsafe { ioctl_fiemap(file_descriptor, c_fiemap_info as *mut IoctlFiemap) }
        .map_err(|error| format!("Error getting file extent map: {}", error))?;

    // return

    Ok(CFileExtentMapResult {
        info: *c_fiemap_info,
