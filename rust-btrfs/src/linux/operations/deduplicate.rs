...
pub fn deduplicate_range(
    ...
    let c_dedupe_range_slice: &mut [IoctlFileDedupeRange] =
        unsafe { slice::from_raw_parts_mut(mem::transmute(c_dedupe_range_buffer.as_mut_ptr()), 1) };

    let c_dedupe_range = &mut c_dedupe_range_slice[0];

    let c_dedupe_range_infos: &mut [IoctlFileDedupeRangeInfo] = unsafe {
        slice::from_raw_parts_mut(
            mem::transmute(c_dedupe_range_infos_buffer.as_mut_ptr()),
    ...
    *c_dedupe_range = IoctlFileDedupeRange {
        src_offset: dedupe_range.src_offset,

        src_length: dedupe_range.src_length,

        dest_count: dedupe_range.dest_infos.len() as u16,
    ...
    for index in 0..dedupe_range.dest_infos.len() {
        let dest_info = &mut dedupe_range.dest_infos[index];

        c_dedupe_range_infos[index] = IoctlFileDedupeRangeInfo {
            dest_fd: dest_info.dest_fd,

            dest_offset: dest_info.dest_offset,
    ...
    unsafe { ioctl_file_dedupe_range(file_descriptor, c_dedupe_range) }
        .map_err(|error| format!("Dedupe ioctl returned {}", error))?;
    ...
    for index in 0..dedupe_range.dest_infos.len() {
        ...
        let c_dedupe_range_info = &c_dedupe_range_infos[index];

        dest_info.bytes_deduped = c_dedupe_range_info.bytes_deduped;

        dest_info.status = match c_dedupe_range_info.status {
            FILE_DEDUPE_RANGE_SAME => DedupeRangeStatus::Same,

            FILE_DEDUPE_RANGE_DIFFERS => DedupeRangeStatus::Differs,
...
pub fn deduplicate_files_with_source<AsPath1: AsRef<Path>, AsPath2: AsRef<Path>>(
    ...
    let mut dedupe_range = DedupeRange {
        src_offset: 0,

        src_length: source_file_metadata.len(),

        dest_infos: target_file_descriptors
            .iter()
            .map(|target_file_descriptor| DedupeRangeDestInfo {
...
pub fn deduplicate_files<AsPath: AsRef<Path>>(filenames: &[AsPath]) -> Result<(), String> {
    ...
    let (source_filename, dest_filenames) = filenames.split_at(1);
    ...
    deduplicate_files_with_source(source_filename, &dest_filenames)
}
