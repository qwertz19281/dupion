...
impl From<u64> for BtrfsTreeId {
    fn from(object_id: u64) -> BtrfsTreeId {
        BtrfsTreeId::new(object_id)
    }
}
...
impl Debug for BtrfsTreeId {
    fn fmt(&self, formatter: &mut Formatter) -> Result<(), FmtError> {
        if let Some(tree_name) = BTRFS_TREE_NAMES.get(&self.object_id()) {
            ...
            formatter.write_fmt(format_args!("BtrfsTreeId ({})", self.object_id()))
        }
    }
}
...
impl Display for BtrfsTreeId {
    fn fmt(&self, formatter: &mut Formatter) -> Result<(), FmtError> {
        if let Some(tree_name) = BTRFS_TREE_NAMES.get(&self.object_id()) {
            formatter.write_fmt(format_args!("{} = {}", tree_name, self.object_id()))
        } else {
            formatter.write_fmt(format_args!("{}", self.object_id()))
        }
    }
}

impl From<BtrfsTreeId> for u64 {
    fn from(btrfs_tree_id: BtrfsTreeId) -> u64 {
        let BtrfsTreeId(object_id) = btrfs_tree_id;
...
impl From<BtrfsTreeId> for String {
    fn from(btrfs_tree_id: BtrfsTreeId) -> String {
...
lazy_static! {
    static ref BTRFS_TREE_NAMES: HashMap<u64, &'static str> = {
        ...
        hash_map.insert(BTRFS_ROOT_TREE_OBJECT_ID, "ROOT");

        hash_map.insert(BTRFS_EXTENT_TREE_OBJECT_ID, "EXTENT");

        hash_map.insert(BTRFS_CHUNK_TREE_OBJECT_ID, "CHUNK");

        hash_map.insert(BTRFS_DEV_TREE_OBJECT_ID, "DEV");

        hash_map.insert(BTRFS_FS_TREE_OBJECT_ID, "FS");
