...
impl<'a> BtrfsSuperblock<'a> {
    ...
    pub fn for_bytes(bytes: &'a [u8]) -> BtrfsSuperblock<'a> {
        Self::new(BtrfsSuperblockData::for_bytes(bytes))
    }

    pub fn system_chunks(self) -> BtrfsSuperblockSystemChunks<'a> {
        BtrfsSuperblockSystemChunks::new(&self.data)
    }
    ...
    pub fn system_logical_to_physical(&self, logical_address: u64) -> Option<BtrfsPhysicalAddress> {
        for system_chunk in self.system_chunks() {
            if logical_address < system_chunk.key().offset()
                || logical_address
                    >= (system_chunk.key().offset() + system_chunk.data().chunk_size())
            {
                continue;
            }

            let system_chunk_stripe = system_chunk.stripes()[0];

            return Some(BtrfsPhysicalAddress::new(
                system_chunk_stripe.device_id(),
                logical_address - system_chunk.key().offset() + system_chunk_stripe.offset(),
            ));
        }
...
impl<'a> Debug for BtrfsSuperblock<'a> {
    fn fmt(&self, formatter: &mut Formatter) -> Result<(), FmtError> {
        let mut debug_struct = formatter.debug_struct("BtrfsSuperblock");
