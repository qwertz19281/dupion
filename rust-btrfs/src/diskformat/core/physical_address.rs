...
impl Debug for BtrfsPhysicalAddress {
    fn fmt(&self, formatter: &mut Formatter) -> Result<(), FmtError> {
        let mut debug_struct = formatter.debug_struct("BtrfsPhysicalAddress");

        debug_struct.field("device_id", &self.device_id());

        debug_struct.field("offset", &format!("0x{:x}", self.offset()));
...
impl Display for BtrfsPhysicalAddress {
    fn fmt(&self, formatter: &mut Formatter) -> Result<(), FmtError> {
        formatter.write_fmt(format_args!("{}/0x{:x}", self.device_id(), self.offset()))
    }
}
