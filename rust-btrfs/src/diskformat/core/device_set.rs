...
impl<'a> BtrfsDeviceSet<'a> {
    pub fn new(datas: &[&'a [u8]]) -> Result<BtrfsDeviceSet<'a>, String> {
        ...
        let mut superblocks: Vec<BtrfsSuperblock<'a>> = Vec::new();

        for data in datas {
            let btrfs_device = BtrfsDevice::new(data)?;

            if devices.contains_key(&btrfs_device.device_id()) {
                return Err(format!(
            ...
            superblocks.push(BtrfsSuperblock::for_bytes(
                &data[btrfs_device.superblock_offset()
                    ..btrfs_device.superblock_offset() + mem::size_of::<BtrfsSuperblockData>()],
            ));

            devices.insert(btrfs_device.device_id(), btrfs_device);
        }
    ...
    pub fn device(&'a self, device_id: u64) -> Option<&'a BtrfsDevice<'a>> {
        self.devices.get(&device_id)
    }

    pub fn system_logical_to_physical(&self, logical_address: u64) -> Option<BtrfsPhysicalAddress> {
        self.superblock.system_logical_to_physical(logical_address)
    }
    ...
    pub fn slice_at_physical_address(
        ...
        let device = self
            .devices
            .get(&physical_address.device_id())
            .ok_or(format!(
                "Device not found: {}",
    ...
    pub fn node_at_physical_address(
        ...
        device.node_at(physical_address.offset())
    }
