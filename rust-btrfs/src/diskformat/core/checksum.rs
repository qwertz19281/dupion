...
pub fn btrfs_crc32c(bytes: &[u8]) -> u32 {
    crc32::checksum_castagnoli(bytes)
}

pub fn btrfs_crc32_linux(bytes: &[u8]) -> u32 {
    !crc32::update(1, &crc32::CASTAGNOLI_TABLE, bytes)
}
...
impl BtrfsChecksum {
    ...
    pub fn bytes_truncated(&self) -> &[u8] {
        if self.bytes.iter().skip(4).all(|byte| *byte == 0) {
            &self.bytes[0..4]
        } else {
            &self.bytes
        }
...
impl Debug for BtrfsChecksum {
    fn fmt(&self, formatter: &mut Formatter) -> Result<(), FmtError> {
        formatter.write_fmt(format_args!("BtrfsChecksum ({})", self.to_string()))
    }
}
...
impl From<u32> for BtrfsChecksum {
    fn from(value: u32) -> BtrfsChecksum {
        BtrfsChecksum {
            bytes: [
