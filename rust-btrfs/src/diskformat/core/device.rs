...
impl<'a> BtrfsDevice<'a> {
    pub fn new(data: &[u8]) -> Result<BtrfsDevice, String> {
        // find superblock
        ...
        let (superblock_data, superblock_offset) = {
            ...
            for superblock_offset in [0x1_0000, 0x400_0000, 0x40_0000_0000, 0x4_0000_0000_0000] {
                ...
                let superblock_data: &BtrfsSuperblockData =
                    unsafe { mem::transmute(&data[superblock_offset]) };

                if superblock_data.magic != BTRFS_MAGIC {
                    continue;
                }

                superblock_datas.push((superblock_data, superblock_offset));
            }
    ...
    pub unsafe fn struct_at<Type>(&self, offset: u64) -> &Type {
        mem::transmute(&self.data[offset as usize])
    }

    pub fn node_at(&self, offset: u64) -> Result<BtrfsNode, String> {
        self.slice_at(offset as usize, self.superblock().node_size() as usize)
            .ok_or(format!(
            ...
            .and_then(|node_bytes| {
                BtrfsNode::from_bytes(
                    BtrfsPhysicalAddress::new(self.device_id(), offset as u64),
                    node_bytes,
                )
            })
    }
    ...
    pub fn superblock(&self) -> BtrfsSuperblock {
        BtrfsSuperblock::new(unsafe { &*self.superblock_data })
    }
