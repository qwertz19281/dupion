...
impl<'a> BtrfsFilesystem<'a> {
    pub fn open_try_backups(
        ...
        Self::open(output, devices).or_else(|error| {
            output_message!(output, "Error opening filesystem: {}", error);
            ...
            for backup_index in 0..4 {
                ...
                match Self::open_backup(output, devices, backup_index) {
                    Ok(filesystem) => return Ok(filesystem),

                    Err(error) => output_message!(
                        output,
    ...
    pub fn open(
        ...
        let output_job = output_job_start!(output, "Reading chunk tree");

        let chunk_tree = BtrfsChunkTree::new(&devices)?;
        ...
        let physical_address = chunk_tree
            .logical_to_physical_address(devices.superblock().chunk_tree_logical_address())
            .unwrap();
        ...
        let output_job = output_job_start!(output, "Reading root tree");

        let root_tree = BtrfsRootTree::read_logical_address(
            output,
            &devices,
            &chunk_tree,
        ...
        let filesystem_trees = {
            let output_job = output_job_start!(output, "Reading filesystem trees");

            let filesystem_trees =
                Self::read_filesystem_trees(output, &devices, &chunk_tree, &root_tree)?;
    ...
    pub fn open_backup(
        ...
        let root_backup = devices.superblock().root_backups()[backup_index];
    ...
    pub fn read_filesystem_trees<'b>(
        ...
        let mut filesystem_trees: HashMap<u64, BtrfsFilesystemTree<'a>> = HashMap::new();

        for root_item in root_tree.subvolume_root_items() {
            filesystem_trees.insert(
                root_item.object_id(),
                Self::read_filesystem_tree(output, devices, chunk_tree, &root_item)?,
            );
        }
    ...
    pub fn read_filesystem_tree<'b>(
        ...
        let output_job =
            output_job_start!(output, "Reading filesystem tree {}", root_item.object_id());

        let filesystem_tree = BtrfsFilesystemTree::read_logical_address(
            output,
            devices,
            chunk_tree,
    ...
    pub fn filesystem_tree(&'a self, tree_id: u64) -> Option<&'a BtrfsFilesystemTree<'a>> {
        self.filesystem_trees.get(&tree_id)
    }
    ...
    pub fn root_item(&'a self, tree_id: u64) -> Option<BtrfsRootItem<'a>> {
        self.root_tree.root_item(tree_id)
    }
    ...
    pub fn logical_to_physical_address(
        ...
        self.chunk_tree.logical_to_physical_address(logical_address)
    }
    ...
    pub fn slice_at_physical_address(
        ...
        device
            .slice_at(physical_address.offset() as usize, size)
            .ok_or(format!(
    ...
    pub fn slice_at_logical_address(
        ...
        let physical_address = self
            .logical_to_physical_address(logical_address)
            .ok_or(format!(
                "Logical address not valid: 0x{:x}",
                logical_address
            ))?;

        self.slice_at_physical_address(physical_address, size)
    }
    ...
    pub fn subvolume_path(
        ...
        let parent_filesystem_tree = self.filesystem_tree(root_backref.offset()).ok_or(format!(
            "Can't find parent filesystem for subvolume {}",
            root_backref.object_id()
        ))?;

        let mut path_parts: LinkedList<PathBuf> = LinkedList::new();

        path_parts.push_front(PathBuf::from(OsStr::from_bytes(root_backref.name())));

        let mut object_id = root_backref.directory_id();

        loop {
            let parent_inode_ref = parent_filesystem_tree
                .inode_refs(object_id)
                .into_iter()
                .next()
                .ok_or(format!("Can't find inode ref {}", object_id))?
                .entries()
                .next()
                .unwrap();
            ...
            path_parts.push_front(PathBuf::from(OsStr::from_bytes(parent_inode_ref.name())));

            object_id = parent_inode_ref.offset();
        }
