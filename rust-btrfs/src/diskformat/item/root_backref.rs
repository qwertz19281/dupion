...
impl<'a> BtrfsRootBackref<'a> {
    pub fn from_bytes(
        ...
        if data_bytes.len()
            != (mem::size_of::<BtrfsRootRefData>() + root_ref.name_length() as usize)
        {
            return Err(format!(
    ...
    pub fn data(&self) -> &BtrfsRootRefData {
        unsafe { &*(self.data_bytes.as_ptr() as *const BtrfsRootRefData) }
    }
    ...
    pub fn name(&'a self) -> &'a [u8] {
        &self.data_bytes[mem::size_of::<BtrfsRootRefData>()
            ..mem::size_of::<BtrfsRootRefData>() + self.name_length() as usize]
    }
...
impl<'a> Debug for BtrfsRootBackref<'a> {
    fn fmt(&self, formatter: &mut Formatter) -> Result<(), FmtError> {
        let mut debug_struct = formatter.debug_struct("BtrfsRootBackref");

        debug_struct.field("key", &NakedString::from(self.key().to_string_decimal()));

        debug_struct.field("directory_id", &self.directory_id());
