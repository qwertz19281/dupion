...
impl<'a> BtrfsInodeItem<'a> {
    ...
    pub fn data(&self) -> &BtrfsInodeItemData {
        unsafe { &*(self.data_bytes.as_ptr() as *const BtrfsInodeItemData) }
    }
...
impl<'a> Debug for BtrfsInodeItem<'a> {
    fn fmt(&self, formatter: &mut Formatter) -> Result<(), FmtError> {
        let mut debug_struct = formatter.debug_struct("BtrfsInodeItem");

        self.header().debug_struct(&mut debug_struct);

        self.data().debug_struct(&mut debug_struct);
