...
impl<'a> BtrfsExtentData<'a> {
    pub fn from_bytes(
        ...
        if extent_data.inline() {
            if data_bytes.len() < mem::size_of::<BtrfsExtentDataData>() - mem::size_of::<u64>() * 4
            {
                return Err(format!(
    ...
    pub fn data(&self) -> &BtrfsExtentDataData {
        unsafe { &*(self.data_bytes.as_ptr() as *const BtrfsExtentDataData) }
    }
    ...
    pub fn inline_data(&self) -> Result<Option<Cow<[u8]>>, String> {
        if self.inline() {
            let raw_data_start = mem::size_of::<BtrfsExtentDataData>() - 4 * mem::size_of::<u64>();

            let raw_data_end = self.data_bytes.len();
            ...
            btrfs_decompress_pages(self.compression(), raw_data, self.logical_data_size()).map(Some)
        } else {
            Ok(None)
        }
    }
