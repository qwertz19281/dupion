    ...
    (
        ...
        impl<'a> $container<'a> {
            pub fn from_bytes(
                ...
                Ok($container { header, data_bytes })
            }

            pub fn entries(&self) -> $iterator<'a> {
                $iterator::new(self.header, self.data_bytes)
            }
        }
        ...
        impl<'a> Debug for $container<'a> {
            fn fmt(&self, formatter: &mut Formatter) -> Result<(), FmtError> {
                let mut debug_struct = formatter.debug_struct(stringify!($container));
                ...
                debug_struct.field("entries", &$entries::new(self));
